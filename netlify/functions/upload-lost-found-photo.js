// Upload Lost & Found photo into the tenant's private storage namespace.
  ...
  try {
    ...
    if (tag.toLowerCase() !== 'pending' || itemId) {
      if (!itemId && tag.toLowerCase() !== 'pending') {
        ...
        const rows = await itemRes.json(); if (!rows.length) return { statusCode: 404, headers, body: JSON.stringify({ error: 'Lost & Found item not found' }) }; item = rows[0];
      } else {
        ...
        if (!itemRes.ok) return { statusCode: 500, headers, body: JSON.stringify({ error: 'Unable to validate Lost & Found item' }) };
        const rows = await itemRes.json(); if (!rows.length) return { statusCode: 404, headers, body: JSON.stringify({ error: 'Lost & Found item not found' }) }; item = rows[0];
        if (tag.toLowerCase() !== 'pending' && item.tag_number !== tag && item.tag_number !== `${tag.replace(/-sig$/i, '')}`) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Upload tag does not match Lost & Found item' }) };
      }
    }
    ...
    for (const image of images) {
      ...
      const response = await fetch(`${supabaseUrl}/storage/v1/object/${BUCKET}/${path}`, { method: 'POST', headers: { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': decoded.mimeType, 'x-upsert': 'false' }, body: decoded.buffer });
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ success: true, paths }) };
