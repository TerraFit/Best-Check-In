...
export const handler = async function(event) {
  ...
  try {
    ...
    const response = await fetch(`${supabaseUrl}/rest/v1/conversations?${params.toString()}`, {
    ...
    const conversationsWithUnread = await Promise.all(conversations.map(async (conv) => {
      const unreadParams = new URLSearchParams({
        ...
        is_read: 'eq.false',
        sender_type: 'neq.admin',
        select: 'id'
      });
      const unreadResponse = await fetch(`${supabaseUrl}/rest/v1/messages?${unreadParams.toString()}`, {
        headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}` }
      });
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ conversations: conversationsWithUnread, total: conversationsWithUnread.length })
    };
  } catch (error) {
