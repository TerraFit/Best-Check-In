...
function canManageStaff(decoded) {
  ...
  if (['business_owner','general_manager','administration','supervisor','super_admin'].includes(role)) return true;
  const perms = Array.isArray(meta.permission_set) ? meta.permission_set : [];
  return perms.includes('canManageStaff');
}
exports.handler = async function (event) {
  const headers = {'Content-Type':'application/json','Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'Content-Type, Authorization','Access-Control-Allow-Methods':'GET, POST, PUT, PATCH, DELETE, OPTIONS'};
  if (event.httpMethod === 'OPTIONS') return {statusCode:204,headers,body:''};
  const supabaseUrl = process.env.SUPABASE_URL;
  const supabaseKey = process.env.SUPABASE_SERVICE_KEY;
  try {
    const authHeader = event.headers.authorization;
    if (!authHeader) return {statusCode:401,headers,body:JSON.stringify({error:'No authorization token provided'})};
    const token = authHeader.replace('Bearer ','').trim();
    let decoded;
    try { decoded = jwt.verify(token, process.env.SUPABASE_JWT_SECRET); }
    catch (verifyErr) { return {statusCode:401,headers,body:JSON.stringify({error:'Invalid token: '+verifyErr.message})}; }
    if (!canManageStaff(decoded)) return {statusCode:403,headers,body:JSON.stringify({error:'Missing permission: canManageStaff'})};
    const businessId = decoded.user_metadata?.business_id;
    if (!businessId) return {statusCode:403,headers,body:JSON.stringify({error:'Token missing business ID'})};

    if (event.httpMethod === 'GET') {
      const response = await fetch(`${supabaseUrl}/rest/v1/employees?business_id=eq.${businessId}&select=*&order=created_at.desc`,{headers:{apikey:supabaseKey,Authorization:`Bearer ${supabaseKey}`}});
    ...
    if (event.httpMethod === 'POST') {
      ...
      const insertResponse = await fetch(`${supabaseUrl}/rest/v1/employees`,{method:'POST',headers:{apikey:supabaseKey,Authorization:`Bearer ${supabaseKey}`,'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify([insertData])});
      if (!insertResponse.ok) {
        const errText = await insertResponse.text();
        if (/additional_departments/i.test(errText)) {
          delete insertData.additional_departments;
          const retry = await fetch(`${supabaseUrl}/rest/v1/employees`,{method:'POST',headers:{apikey:supabaseKey,Authorization:`Bearer ${supabaseKey}`,'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify([insertData])});
    ...
    if (event.httpMethod === 'PUT' || event.httpMethod === 'PATCH') {
      ...
      const response = await fetch(`${supabaseUrl}/rest/v1/employees?id=eq.${id}&business_id=eq.${businessId}`,{method:'PATCH',headers:{apikey:supabaseKey,Authorization:`Bearer ${supabaseKey}`,'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify(updateData)});
    ...
    if (event.httpMethod === 'DELETE') {
      ...
      if (!id) return {statusCode:400,headers,body:JSON.stringify({error:'Employee ID required'})};
      const response = await fetch(`${supabaseUrl}/rest/v1/employees?id=eq.${id}&business_id=eq.${businessId}`,{method:'DELETE',headers:{apikey:supabaseKey,Authorization:`Bearer ${supabaseKey}`} });
