...
export const handler = async (event) => {
  ...
  try {
    ...
    const lim = Math.min(parseInt(q.limit || '200', 10) || 200, 500), r = await fetch(`${url}/rest/v1/lost_and_found?${filter}&select=*&order=found_date.desc,created_at.desc&limit=${lim}`, { headers: sh });
    if (!r.ok) return { statusCode: r.status, headers, body: JSON.stringify({ error: await r.text() }) }; const items = await r.json();
    const sr = await fetch(`${url}/rest/v1/lost_and_found?business_id=eq.${encodeURIComponent(businessId)}&select=id,status,found_date,returned_at,created_at,photo_urls`, { headers: sh }), all = sr.ok ? await sr.json() : [];
  ...
  } catch (error) { console.error('get-lost-found-items fatal:', error); return { statusCode: 500, headers, body: JSON.stringify({ error: error.message || 'Failed to fetch lost & found items' }) }; }
};
