...
export const handler = async function(event) {
  ...
  if (!supabaseUrl || !supabaseKey) return { statusCode: 500, headers, body: JSON.stringify({ error: 'Server configuration error: Missing Supabase credentials' }) };

  try {
    let businessId;
    try { businessId = JSON.parse(event.body || '{}').businessId; }
    ...
    const fetchResponse = await fetch(`${supabaseUrl}/rest/v1/businesses?id=eq.${encodeURIComponent(businessId)}&select=*`, {
      headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}` }
    });
    if (!fetchResponse.ok) return { statusCode: 502, headers, body: JSON.stringify({ error: 'Failed to load business' }) };
    ...
    const updateResponse = await fetch(`${supabaseUrl}/rest/v1/businesses?id=eq.${encodeURIComponent(businessId)}`, {
      method: 'PATCH',
    ...
    if (!updateResponse.ok) return { statusCode: 500, headers, body: JSON.stringify({ error: 'Failed to approve business' }) };
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, message: 'Business approved successfully', business: { id: updatedBusiness.id, trading_name: updatedBusiness.trading_name, email: updatedBusiness.email, status: updatedBusiness.status }, checkInUrl, qrCode: qrCodeDataUrl, emailSent, verificationLink })
    };
  } catch (error) {
...
function generateWelcomeEmail(businessName, verificationLink, checkInUrl, qrCodeDataUrl) {
  return `
    ...
    <body><div class="container"><div class="header"><h1>Welcome to FastCheckin!</h1></div><div class="content"><h2 style="color:#333;margin-bottom:20px">Hello ${businessName}!</h2><p style="color:#555;line-height:1.6">Your business has been approved! You're now ready to start using FastCheckin for digital guest check-ins.</p>
    <div class="qr-box">${qrCodeDataUrl ? `<img src="${qrCodeDataUrl}" alt="QR Code">` : '<p>QR code generation failed. Please log in to download your QR code.</p>'}<p style="color:#333;font-weight:bold;margin:10px 0">Scan to check in guests</p><p style="color:#777;font-size:14px;word-break:break-all">${checkInUrl}</p></div>
    <div class="next-steps"><h3 style="color:#0284c7;margin:0 0 10px">✨ Next Steps:</h3><ol style="color:#555;line-height:1.8;margin:0;padding-left:20px"><li><strong>Verify your email</strong> - Click the button below to set up your password</li><li><strong>Download your QR code</strong> - Print and display at your reception</li><li><strong>Customize your check-in page</strong> - Add your logo and colors</li><li><strong>Start accepting guest check-ins</strong> - Guests scan and complete registration</li></ol></div>
    <div style="text-align:center"><a href="${verificationLink}" class="button">✓ Verify Email & Set Password</a></div><p style="font-size:12px;color:#6b7280;text-align:center;margin-top:30px">This verification link expires in 48 hours.</p></div>
    <div class="footer"><p>FastCheckin - Seamless Check-in, Smarter Stay</p><p><a href="https://fastcheckin.co.za" style="color:#f59e0b">www.fastcheckin.co.za</a></p><p>© ${new Date().getFullYear()} FastCheckin. All rights reserved.</p></div></div></body></html>`;
}
