...
export const handler = async function(event) {
  ...
  try {
    ...
    const businessScope = scope.businessId;

    const readHeaders = { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}`, Accept: 'application/json' };
    const bookingResponse = await fetch(`${supabaseUrl}/rest/v1/bookings?id=eq.${encodeURIComponent(bookingId)}&business_id=eq.${encodeURIComponent(businessScope)}&select=id,business_id`, { headers: readHeaders });
