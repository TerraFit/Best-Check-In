...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!ALLOWED.has(service_type) || !Number(target_minutes)) return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: 'valid service_type and target_minutes are required' }) };
    ...
    let res;
    if (existing?.id) res = await fetch(`${supabaseUrl}/rest/v1/housekeeping_service_targets?id=eq.${encodeURIComponent(existing.id)}`, { method: 'PATCH', headers: write, body: JSON.stringify(payload) });
    else res = await fetch(`${supabaseUrl}/rest/v1/housekeeping_service_targets`, { method: 'POST', headers: write, body: JSON.stringify(payload) });
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ success: false, error: error.message || 'Failed to save service target' }) };
  }
};
