...
exports.handler = async (event) => {
  ...
  if (!token) return response(401, { error: 'No authorization token provided' });
  let decoded;
  try { decoded = jwt.verify(token, process.env.SUPABASE_JWT_SECRET); }
  catch (error) { return response(401, { error: error.name === 'TokenExpiredError' ? 'Token has expired' : 'Invalid token signature' }); }

  const metadata = decoded.user_metadata || {};
  const businessId = metadata.business_id;
  const employeeId = metadata.employee_id || decoded.sub;
  ...
  const read = { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` };
  ...
  try {
    const [arrivalsRes, departuresRes, stayoversRes] = await Promise.all([
    ...
    for (const result of [arrivalsRes, departuresRes, stayoversRes]) if (!result.ok) throw new Error(`Bookings query failed with HTTP ${result.status}`);
    const [arrivals, departures, stayovers] = await Promise.all([arrivalsRes.json(), departuresRes.json(), stayoversRes.json()]);
    ...
    return response(500, { error: 'Unable to load employee overview' });
  }
};
