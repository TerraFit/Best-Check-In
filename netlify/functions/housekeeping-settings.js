...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const res = await fetch(`${supabaseUrl}/rest/v1/housekeeping_settings?business_id=eq.${encodeURIComponent(businessId)}&select=*`, { headers: read });
    ...
    if (body.policy !== undefined && !VALID_POLICIES.has(body.policy)) {
      return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: `Invalid policy "${body.policy}". Allowed values: eco, standard, premium, custom` }) };
    }
    ...
    let settings;
    if (existing[0]?.id) {
      const res = await fetch(`${supabaseUrl}/rest/v1/housekeeping_settings?id=eq.${encodeURIComponent(existing[0].id)}`, { method: 'PATCH', headers: write, body: JSON.stringify(payload) });
      const rows = await res.json();
      if (!res.ok) return { statusCode: res.status, headers, body: JSON.stringify({ success: false, error: rows }) };
      settings = rows[0];
    } else {
      const res = await fetch(`${supabaseUrl}/rest/v1/housekeeping_settings`, { method: 'POST', headers: write, body: JSON.stringify({ ...DEFAULTS, ...payload }) });
      const rows = await res.json();
      if (!res.ok) return { statusCode: res.status, headers, body: JSON.stringify({ success: false, error: rows }) };
      settings = rows[0];
    }
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ success: false, error: error.message || 'Failed to load housekeeping settings' }) };
  }
};
