...
export function buildSnapshotPdfPayload(summary) {
  ...
  const page1 = [];
  addHeader(page1, businessName, meta, 'Executive overview');
  ...
  const page2 = [];
  addHeader(page2, businessName, meta, 'Acquisition intelligence');
  ...
  drawReferralMatrix(page2, summary.referralByCountry || [], 50, 390, 495);
  addReferralInsights(page2, summary.referralByCountry || [], 50, 145);
  page2.push(textCmd('Insight percentages are calculated within each guest market, not against the overall booking total.', 50, 52, 6.5, '#94a3b8'));

  const page3 = [];
  addHeader(page3, businessName, meta, 'Stay and room performance');
