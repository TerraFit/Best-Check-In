...
export const handler = async (event) => {
  ...
  try {
    let body;
    try { body = JSON.parse(event.body || '{}'); } catch { return { statusCode: 400, headers, body: JSON.stringify({ error: 'Malformed JSON' }) }; }
    ...
    if (body.status === 'archived' && !requireBusinessPermission(a.principal, 'canDisposeLostFound')) return authFailure({ status: 403, error: 'Missing permission: canDisposeLostFound' }, headers);
    ...
    if (body.status && body.status !== current.status) {
      await activity(supabaseUrl, key, { business_id: businessId, item_id: itemId, event_type: body.status === 'archived' ? 'archived' : body.status === 'returned' || body.status === 'collected' ? 'returned' : 'status_change', employee_id: empId, employee_name: empName, from_status: current.status, to_status: body.status, notes: body.note || `Status changed from ${current.status} to ${body.status}` });
