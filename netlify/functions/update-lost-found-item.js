// Update Lost & Found item fields/status with tenant-bound authorization.
...
export const handler = async (event) => {
  ...
  if (!requireBusinessPermission(a.principal, 'canEditLostFound')) {
    return authFailure({ status: 403, error: 'Missing permission: canEditLostFound' }, headers);
  }

  try {
    let body;
    try { body = JSON.parse(event.body || '{}'); } catch {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Malformed JSON' }) };
    }

    const t = resolveTenant(a.principal, body.businessId || body.business_id);
    ...
    // Generic edit must not be a backdoor around the dedicated contact/collection/disposal endpoints.
    // Those endpoints own their corresponding state transitions and handover attribution.
    if (body.status !== undefined && body.status !== current.status) {
      const next = body.status;
      if (next === 'archived') {
        if (!requireBusinessPermission(a.principal, 'canDisposeLostFound')) {
      ...
      if (!allowedNext.has(next)) {
        return { statusCode: 409, headers, body: JSON.stringify({ error: 'Invalid Lost & Found status transition' }) };
      }
    }

    // Cross-resource identifiers are authoritative only when they resolve inside the same tenant.
    // This prevents attaching an item to another business's room or booking by ID.
    if (body.room_id !== undefined && body.room_id !== null && body.room_id !== '') {
      ...
      if (!rooms.length) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Room does not belong to this business' }) };
    }
    ...
    if (body.booking_id !== undefined && body.booking_id !== null && body.booking_id !== '') {
      ...
      if (!bookings.length) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Booking does not belong to this business' }) };
      if (body.room_id && bookings[0].room_id && String(bookings[0].room_id) !== String(body.room_id)) {
        return { statusCode: 400, headers, body: JSON.stringify({ error: 'Booking and room do not match' }) };
      }
    }

    const allowed = [
      'item_name', 'description', 'category', 'found_date', 'time_found', 'room_id', 'room_number', 'room_name',
      'booking_id', 'booking_reference', 'guest_name', 'guest_email', 'guest_phone', 'storage_location', 'storage_detail',
      'condition', 'estimated_value', 'internal_notes', 'notes', 'photo_urls', 'status', 'returned_to',
    ];
    ...
    // Optimistic concurrency: do not overwrite a newer version loaded after this request began.
    const patchRes = await fetch(`${base}&updated_at=eq.${q(current.updated_at)}`, {
      method: 'PATCH',
      headers: sh,
    ...
    if (!updatedRows.length) return { statusCode: 409, headers, body: JSON.stringify({ error: 'Lost & Found item changed before update could be applied' }) };
    const updated = updatedRows[0];

    const empId = a.principal.employeeId || a.principal.userId || null;
    const empName = a.principal.email || null;
    const photosChanged = body.photo_urls !== undefined && JSON.stringify(body.photo_urls || []) !== JSON.stringify(current.photo_urls || []);

    if (body.status && body.status !== current.status) {
      await activity(supabaseUrl, key, {
        business_id: businessId, item_id: itemId,
        event_type: body.status === 'archived' ? 'archived' : 'status_change',
      ...
      await audit(supabaseUrl, key, {
        business_id: businessId, user_id: empId || '00000000-0000-0000-0000-000000000000', user_name: empName || 'System',
        user_role: a.principal.role, action: 'lost_found_status_change',
        ...
        booking_id: current.booking_id || null, guest_name: current.guest_name || null, created_at: new Date().toISOString(),
      });
    } else if (photosChanged) {
      const prev = photoCount(current.photo_urls), next = photoCount(body.photo_urls);
      const notes = next > prev ? `${next - prev} photo(s) added (${next} total)` : next < prev ? `Photos updated (${next} remaining, was ${prev})` : `Photos updated (${next} total)`;
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Failed to update Lost & Found item' }) };
  }
};
