...
export const handler = async (event) => {
  ...
  try {
    ...
    const base = `${supabaseUrl}/rest/v1/lost_and_found?id=eq.${encodeURIComponent(itemId)}&business_id=eq.${encodeURIComponent(businessId)}`;
    const curRes = await fetch(`${base}&select=*`, { headers: sh });
    if (!curRes.ok) return { statusCode: curRes.status, headers, body: JSON.stringify({ error: await curRes.text() }) };
    ...
    const patchRes = await fetch(base, { method: 'PATCH', headers: sh, body: JSON.stringify(updates) });
    if (!patchRes.ok) return { statusCode: patchRes.status, headers, body: JSON.stringify({ error: await patchRes.text() }) };
    const updated = (await patchRes.json())[0];

    const empId = body.employee_id || a.principal.employeeId || a.principal.userId || null;
    const empName = body.employee_name || a.principal.email || null;
    ...
    if (body.status && body.status !== current.status) {
      ...
      await activity(supabaseUrl, key, {
        business_id: businessId, item_id: itemId, event_type: 'photos_added', employee_id: empId, employee_name: empName,
        notes, details: { previous_count: prev, new_count: next },
      });
      await audit(supabaseUrl, key, {
        business_id: businessId, user_id: empId || '00000000-0000-0000-0000-000000000000', user_name: empName || 'System',
        user_role: a.principal.role, action: 'lost_found_photos_updated', description: `Lost & Found ${current.tag_number || itemId}: photos ${prev} → ${next}`,
        details: { item_id: itemId, previous_count: prev, new_count: next }, booking_id: current.booking_id || null,
        guest_name: current.guest_name || null, created_at: new Date().toISOString(),
      });
    } else if (body.note) {
      await activity(supabaseUrl, key, {
        business_id: businessId, item_id: itemId, event_type: 'note_added', employee_id: empId, employee_name: empName, notes: body.note,
      });
    } else {
      await activity(supabaseUrl, key, {
        business_id: businessId, item_id: itemId, event_type: 'updated', employee_id: empId, employee_name: empName,
        notes: 'Item details updated', details: { fields: Object.keys(updates).filter((k) => k !== 'updated_at') },
      });
    }
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ error: error.message || 'Failed to update item' }) };
  }
};
