...
function principalFromEvent(event) {
  const auth = (event.headers?.authorization || event.headers?.Authorization || '').trim();
  if (!auth) return { employeeId: null, employeeName: null };
  try {
    const jwt = require('jsonwebtoken');
    const token = auth.replace(/^Bearer\s+/i, '').trim();
    const decoded = jwt.verify(token, process.env.SUPABASE_JWT_SECRET);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { businessId, taskId, serviceType } = JSON.parse(event.body || '{}');
    if (!businessId || !taskId || !serviceType) return { statusCode: 400, headers, body: JSON.stringify({ error: 'businessId, taskId and serviceType are required' }) };
    if (!Object.prototype.hasOwnProperty.call(DEFAULT_TARGETS, serviceType)) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Unsupported service type' }) };
    ...
    if (task.status !== 'pending') return { statusCode: 409, headers, body: JSON.stringify({ error: `Task is already ${task.status}`, task }) };
