...
test('business directors: missing business id is rejected', async () => { const { handler } = await loadFunction(); const result = await handler(event(businessToken(), null)); assert.equal(result.statusCode, 400); });
