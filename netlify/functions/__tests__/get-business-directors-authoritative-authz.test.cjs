...
test('business directors: employee cannot substitute another tenant', async () => { const { handler } = await loadFunction(); const result = await handler(event(employeeToken('biz-a'), 'biz-b')); assert.equal(result.statusCode, 403); });
...
test('business directors: platform finance cannot access business directors', async () => { const { handler } = await loadFunction(); const result = await handler(event(platformToken('platform_finance'))); assert.equal(result.statusCode, 403); });
...
test('business directors: missing business id is rejected', async () => { const { handler } = await loadFunction(); const result = await handler(event(businessToken(), undefined)); assert.equal(result.statusCode, 400); });
