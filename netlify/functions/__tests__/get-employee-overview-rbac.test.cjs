...
function event(jwtToken, businessId = BUSINESS_ID) {
  return {
    ...
    queryStringParameters: { businessId },
  };
}

async function loadHandler() {
  return (await import('../get-employee-overview.js')).handler;
}
...
test('housekeeper receives name, country and room but no phone or food restrictions', async () => {
  ...
  assert.deepEqual(data.capabilities, { guestOverview: true, guestPhone: false, foodRestrictions: false });
...
test('receptionist receives phone and food restrictions', async () => {
  const { result } = await invoke({ role: 'front_desk', department: 'front_office' });
  const data = body(result);
  assert.equal(result.statusCode, 200);
  assert.deepEqual(data.capabilities, { guestOverview: true, guestPhone: true, foodRestrictions: true });
...
test('kitchen department receives food restrictions without phone', async () => {
  const { result } = await invoke({ role: 'Employee (Legacy)', department: 'kitchen' });
  const data = body(result);
  assert.equal(result.statusCode, 200);
  assert.deepEqual(data.capabilities, { guestOverview: true, guestPhone: false, foodRestrictions: true });
  assert.equal('guest_phone' in data.arrivals[0], false);
  assert.equal(data.arrivals[0].food_restrictions.vegan, true);
});
