...
function circleCmd(cx, cy, r, fill = '#ffffff', stroke = null) {
  ...
  if (stroke) out += ` ${rgb(stroke)} RG ${cx + r} ${cy + k} ${cx + k} ${cy + r} ${cx} ${cy + r} c ${cx - k} ${cy + r} ${cx - r} ${cy + k} ${cx - r} ${cy} c ${cx - r} ${cy - k} ${cx - k} ${cy - r} ${cx} ${cy - r} c ${cx + k} ${cy - r} ${cx + r} ${cy - k} ${cx + r} ${cy} c S`;
  return out;
}
...
export function buildVisualPdf(pages, meta = {}) {
  ...
  for (const image of meta.images || []) {
    if (!image?.name || !image?.data || !image.width || !image.height) continue;
    const filter = image.filter === 'DCTDecode' ? '/DCTDecode' : '/FlateDecode';
    const data = Buffer.isBuffer(image.data) ? image.data : Buffer.from(image.data);
    const stream = `<< /Type /XObject /Subtype /Image /Width ${image.width} /Height ${image.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter ${filter} /Length ${data.length} >>\nstream\n`;
    const object = Buffer.concat([Buffer.from(stream, 'latin1'), data, Buffer.from('\nendstream', 'latin1')]);
    const id = addObj(object);
    imageObjects.set(image.name, id);
  }
  ...
  pages.forEach((page) => {
    ...
    if (meta.footer) commands.push(textCmd(meta.footer, 50, 16, 7, '#9ca3af'));
    const stream = commands.join('\n');
    contentIds.push(addObj(`<< /Length ${Buffer.byteLength(stream, 'latin1')} >>\nstream\n${stream}\nendstream`));
  });

  contentIds.forEach((contentId) => {
    const xObjects = {};
    for (const [name, id] of imageObjects.entries()) xObjects[name] = `/${name} ${id} 0 R`;
    const xObjectDict = Object.keys(xObjects).length ? ` /XObject << ${Object.values(xObjects).join(' ')} >>` : '';
    pageIds.push(addObj(`<< /Type /Page /Parent 0 0 R /MediaBox [0 0 ${pageWidth} ${pageHeight}] /Contents ${contentId} 0 R /Resources << /Font << /F1 ${fontId} 0 R /F2 ${fontBoldId} 0 R >>${xObjectDict} >> >>`));
  });

  const kids = pageIds.map((id) => `${id} 0 R`).join(' ');
  const pagesId = addObj(`<< /Type /Pages /Kids [ ${kids} ] /Count ${pageIds.length} >>`);
  pageIds.forEach((pid) => { objects[pid - 1] = Buffer.from(objects[pid - 1].toString('latin1').replace('/Parent 0 0 R', `/Parent ${pagesId} 0 R`), 'latin1'); });
  const catalogId = addObj(`<< /Type /Catalog /Pages ${pagesId} 0 R >>`);

  const objectBuffers = objects.map((obj) => Buffer.isBuffer(obj) ? obj : Buffer.from(String(obj), 'latin1'));
  let pdf = Buffer.from('%PDF-1.4\n', 'latin1');
  const offsets = [0];
  objectBuffers.forEach((obj, i) => {
    offsets.push(pdf.length);
    pdf = Buffer.concat([pdf, Buffer.from(`${i + 1} 0 obj\n`, 'latin1'), obj, Buffer.from('\nendobj\n', 'latin1')]);
  });
  const xrefPos = pdf.length;
  let xref = `xref\n0 ${objectBuffers.length + 1}\n0000000000 65535 f \n`;
  for (let i = 1; i <= objectBuffers.length; i++) xref += `${String(offsets[i]).padStart(10, '0')} 00000 n \n`;
  xref += `trailer\n<< /Size ${objectBuffers.length + 1} /Root ${catalogId} 0 R >>\nstartxref\n${xrefPos}\n%%EOF`;
...
export { textCmd, rectCmd, circleCmd, imageCmd, rgb };
