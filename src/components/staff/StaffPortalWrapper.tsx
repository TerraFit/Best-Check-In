...
// ✅ COMPLETE: Employee Management + Audit Trail + Housekeeping
...
export function StaffPortalWrapper({
  ...
  const [activeTab, setActiveTab] = useState<'employees' | 'audit' | 'housekeeping'>('employees');
  ...
  const playNotificationSound = useCallback(() => {
    try {
      ...
      try {
        const audio = new Audio('data:audio/wav;base64,UklGRnoAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoAAACBhYqFhYWJhYaFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhQ==');
        audio.volume = 0.3;
        audio.play().catch(() => {});
      } catch {}
    }
  ...
  // Get storage key for this business
  const getStorageKey = useCallback(() => {
    return `${LAST_VIEWED_KEY}_${business.id}`;
  }, [business.id]);
  ...
  const getLastViewed = useCallback((): string | null => {
    try {
      ...
      console.warn('Could not get last viewed:', e);
      return null;
    }
  }, [getStorageKey]);
  ...
  const setLastViewed = useCallback(() => {
    try {
      ...
      console.warn('Could not set last viewed:', e);
    }
  }, [getStorageKey]);
  ...
  const fetchPendingTaskCount = useCallback(async () => {
    if (!business.id || isEmployee) return;
    ...
    try {
      ...
      const response = await fetch(
        `/.netlify/functions/get-housekeeping-tasks?businessId=${business.id}&status=pending&limit=1`,
        { headers }
      );
  ...
  }, [business.id, isEmployee, activeTab, checkForNewLogs, setLastViewed]);
  ...
  }, [fetchAuditLogs, fetchPendingTaskCount, isEmployee]);
  ...
  // Handle tab switch
  const handleTabChange = useCallback((tab: 'employees' | 'audit' | 'housekeeping') => {
    setActiveTab(tab);
  ...
  }, [setLastViewed, fetchAuditLogs, fetchPendingTaskCount]);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ============================================================
          TABS - Employee Management + Audit Trail + Housekeeping
          ============================================================ */}
