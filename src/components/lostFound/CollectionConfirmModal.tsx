      ...
      if (canvas) {
        ...
        if (!isBlank) {
          ...
          const urls = await uploadLostFoundPhotos({
            businessId,
            images: [dataUrl],
            tagNumber: `${item.tag_number || item.id}-sig`,
          });
          signatureUrl = urls[0];
        }
      }

      const updated = await collectLostFoundItem({
        businessId,
        itemId: item.id,
        collected_by_name: name.trim(),
        collected_by_id_number: idNumber.trim() || undefined,
    ...
    <div className="fixed inset-0 z-[55] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-5 space-y-4">
        ...
        <p className="text-xs text-stone-500">
          Tag <span className="font-mono font-bold text-amber-700">{item.tag_number}</span> —{' '}
          {item.item_name}
        </p>
        <div>
        ...
        <div>
          ...
          <canvas
            ...
            width={360}
            height={120}
            className="w-full border border-stone-200 rounded-xl touch-none bg-stone-50"
            onMouseDown={start}
            onMouseMove={move}
            onMouseUp={end}
            onMouseLeave={end}
        ...
        <p className="text-[11px] text-stone-400">
          Released by: {employeeName || 'Staff'}
        </p>
        ...
        <button
          ...
          disabled={saving}
          onClick={submit}
          className="w-full py-2.5 text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl disabled:opacity-50"
        >
