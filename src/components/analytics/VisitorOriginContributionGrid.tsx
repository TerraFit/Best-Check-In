    ...
    <section className={overlay ? 'rounded-xl border border-stone-200 bg-white/95 p-3 shadow-xl backdrop-blur-md overflow-hidden' : 'rounded-2xl border border-stone-200 bg-gradient-to-b from-stone-50 to-white p-5 md:p-6'}>
      <div className={`flex items-start justify-between gap-2 ${overlay ? 'mb-3' : 'mb-5'}`}>
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {overlay && <button type="button" onClick={() => setCollapsed(value => !value)} className="inline-flex items-center justify-center rounded-lg p-1.5 text-stone-500 hover:bg-stone-100 hover:text-stone-800" aria-label={collapsed ? 'Expand contribution grid' : 'Collapse contribution grid'} title={collapsed ? 'Expand contribution grid' : 'Collapse contribution grid'}>{collapsed ? <ChevronDown size={15} /> : <ChevronUp size={15} />}</button>}
        </div>
      </div>
      {(!overlay || !collapsed) && (
        <>
