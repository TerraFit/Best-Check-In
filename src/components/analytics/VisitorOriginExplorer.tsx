...
export function VisitorOriginExplorer({ businessId, dateFrom, dateTo, limits, canInteractiveMap = true, isLoading: parentLoading = false }: VisitorOriginExplorerProps) {
  ...
  const gridSubtitle = currentLevel === 'world' ? 'Select a continent to see its country distribution.' : currentLevel === 'countries' ? 'Select a country to see provinces, states, cantons or lands.' : currentLevel === 'regions' ? (isUnitedKingdom(selectedCountry) ? 'Select England, Scotland, Wales or Northern Ireland to continue.' : 'Select an administrative area to see its cities.') : 'Select a city to open detailed visitor insights.';
