...
function aggregateContinents(nodes: OriginNode[]): OriginNode[] {
  ...
  if (alreadyAggregated) {
    const total = nodes.reduce((sum, node) => sum + (Number(node.count) || 0), 0);
    return nodes.map(node => ({ ...node, percentage: total ? ((Number(node.count) || 0) / total) * 100 : 0 })).sort((a, b) => b.count - a.count);
  }

  const buckets = new Map<string, { count: number; code?: string }>();
  nodes.forEach(node => {
    const continent = continentOfCountryName(node.name);
    const existing = buckets.get(continent) || { count: 0 };
    existing.count += Number(node.count) || 0;
    buckets.set(continent, existing);
  });
  const total = Array.from(buckets.values()).reduce((sum, item) => sum + item.count, 0);
  return Array.from(buckets.entries())
    .map(([name, item]) => ({ name, count: item.count, percentage: total ? (item.count / total) * 100 : 0 }))
...
export function VisitorOriginExplorer({ businessId, dateFrom, dateTo, limits, canInteractiveMap = true, isLoading: parentLoading = false }: VisitorOriginExplorerProps) {
  ...
  const loadLevel = useCallback(async (uiLevel: UiLevel, parent: ParentSelection) => {
    ...
    try {
      ...
      if (res.upgradeRequired) {
        setModalTargetTier((res.requiredPlan as SubscriptionTier) || 'growth'); setModalFeatureName(res.error || 'Upgrade required'); setShowUpgradeModal(true); return;
      }
      if (!res.success) {
        setFetchError(res.error || 'Failed to load origins'); setNodes([]); setQualityNote(null); return;
      }
      setNodes(res.nodes || []);
  ...
  const handleContinentClick = (continent: string) => {
    ...
    setSelectedContinent(continent); setSelectedCountry(null); setSelectedRegion(null); setSelectedCity(null); setCurrentLevel('countries');
  ...
  const handleRegionClick = (region: string) => {
    ...
    setSelectedRegion(region); setSelectedCity(null); setCurrentLevel('cities');
    loadLevel('cities', { continent: selectedContinent, country: selectedCountry, region });
  };

  const handleCityClick = (city: string) => {
    setSelectedCity(city); setCurrentLevel('cityDetail');
    loadLevel('cityDetail', { continent: selectedContinent, country: selectedCountry, region: selectedRegion, city });
  };
  ...
  const handleBack = () => {
    ...
    if (currentLevel === 'cities') {
      if (selectedRegion) { setSelectedRegion(null); setCurrentLevel('regions'); loadLevel('regions', { continent: selectedContinent, country: selectedCountry }); }
  ...
  const jumpToContinent = () => {
    if (!selectedContinent) return;
    setSelectedCountry(null); setSelectedRegion(null); setSelectedCity(null); setCurrentLevel('countries');
    loadLevel('countries', { continent: selectedContinent });
  };
  const jumpToCountry = () => {
    if (!selectedCountry) return;
    setSelectedRegion(null); setSelectedCity(null);
    if (limits.canViewRegions) { setCurrentLevel('regions'); loadLevel('regions', { continent: selectedContinent, country: selectedCountry }); }
    else { setCurrentLevel('cities'); loadLevel('cities', { continent: selectedContinent, country: selectedCountry }); }
  };
  const jumpToRegion = () => {
  ...
  const gridLevel = currentLevel === 'world' ? 'continents' : currentLevel;
  const gridTitle = currentLevel === 'world'
    ? 'Bookings by continent'
    : currentLevel === 'countries'
      ? `Bookings by country · ${selectedContinent || 'Continent'}`
      : currentLevel === 'regions'
        ? `Bookings by province / state / region · ${selectedCountry || ''}`
        : `Bookings by city · ${selectedRegion || selectedCountry || ''}`;
  const gridSubtitle = currentLevel === 'world'
    ? 'Select a continent to see its country distribution.'
    : currentLevel === 'countries'
      ? 'Select a country to see provinces, states, cantons or lands.'
      : currentLevel === 'regions'
        ? 'Select an administrative area to see its cities.'
        : 'Select a city to open detailed visitor insights.';
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-stone-200 overflow-hidden transition-all duration-300">
      ...
      <div className="bg-stone-100/40 px-6 py-2.5 border-b border-stone-100 flex flex-wrap items-center gap-2 text-xs font-mono text-stone-400">
        <button type="button" className={currentLevel === 'world' ? 'text-orange-500 font-bold' : 'hover:text-stone-600'} onClick={handleHome}>World</button>
        {selectedContinent && <><span>›</span><button type="button" className={currentLevel === 'countries' ? 'text-orange-500 font-bold' : 'hover:text-stone-600'} onClick={jumpToContinent}>{selectedContinent}</button></>}
        {selectedCountry && <><span>›</span><button type="button" className={currentLevel === 'regions' ? 'text-orange-500 font-bold' : 'hover:text-stone-600'} onClick={jumpToCountry}>{selectedCountry}</button></>}
        {selectedRegion && <><span>›</span><button type="button" className={currentLevel === 'cities' ? 'text-orange-500 font-bold' : 'hover:text-stone-600'} onClick={jumpToRegion}>{selectedRegion}</button></>}
        {selectedCity && <><span>›</span><span className="text-orange-500 font-bold">{selectedCity}</span></>}
        <span className="ml-auto rounded-md bg-stone-900 px-2 py-1 text-[9px] font-bold tracking-wider text-orange-300 uppercase">{currentLevel === 'world' ? 'WORLD' : currentLevel === 'countries' ? 'COUNTRIES' : currentLevel === 'regions' ? 'PROVINCES / STATES' : currentLevel === 'cities' ? 'CITIES' : 'CITY INSIGHTS'}</span>
      </div>

      <div className="p-6 space-y-6">
        {fetchError && <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{fetchError}</div>}
        {qualityNote && !fetchError && <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">{qualityNote}</div>}

        {currentLevel !== 'cityDetail' ? (
          <div className="relative h-[500px] md:h-[560px]">
            <GeographicMapViewport
              level={geoLevel as 'world' | 'continents' | 'countries' | 'regions' | 'cities'}
              nodes={displayNodes.map(node => ({ name: node.name, count: node.count, percentage: node.percentage, code: node.code }))}
              ...
              overlayInset={320}
            />
            <div className="absolute left-3 top-3 z-30 hidden w-[320px] md:block">
              <VisitorOriginContributionGrid level={gridLevel} nodes={gridNodes} title={gridTitle} subtitle={gridSubtitle} onSelect={gridSelect} overlay />
            </div>
          </div>
        ) : (
          selectedCity && <CityInsightPanel cityName={selectedCity} regionName={selectedRegion} countryName={selectedCountry} data={cityPanel} isLoading={isBusy} onBack={handleBack} />
        )}
