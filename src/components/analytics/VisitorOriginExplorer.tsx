...
function toApiLevel(ui: UiLevel): DrillLevel { if (ui === 'world') return 'world'; if (ui === 'continents') return 'continent'; if (ui === 'countries') return 'country'; if (ui === 'regions') return 'region'; return 'city'; }
...
function continentOfCountryName(country: string) { return country ? COUNTRY_TO_CONTINENT[country.trim()] || 'Other' : 'Other'; }
...
export function VisitorOriginExplorer({ businessId, dateFrom, dateTo, limits, canInteractiveMap = true, isLoading: parentLoading = false }: VisitorOriginExplorerProps) {
  ...
  const jumpTo = (target: UiLevel) => { if (target === 'world') handleHome(); else if (target === 'continents') { setSelectedCountry(null); setSelectedRegion(null); setSelectedCity(null); setCurrentLevel('continents'); loadLevel('continents', {}); } else if (target === 'countries' && selectedContinent) { setSelectedRegion(null); setSelectedCity(null); setCurrentLevel('countries'); loadLevel('countries', { continent: selectedContinent }); } else if (target === 'regions' && selectedCountry) { setSelectedCity(null); setCurrentLevel('regions'); loadLevel('regions', { continent: selectedContinent, country: selectedCountry }); } };
  const isBusy = loading || parentLoading;

  if (!interactive) return <div className="bg-white rounded-3xl shadow-xl border border-stone-200 overflow-hidden"><div className="px-6 py-5 border-b border-stone-100 bg-stone-50/50 flex items-center gap-3"><div className="p-2 bg-stone-300 rounded-2xl text-white"><Globe2 size={22} /></div><div><h3 className="text-lg font-extrabold text-stone-900">{t('reports_visitor_origin_explorer')}</h3><p className="text-xs text-stone-400 mt-0.5">{t('reports_upgrade_map_growth')}</p></div></div><div className="p-10 text-center space-y-4"><p className="text-stone-600 text-sm max-w-md mx-auto">Unlock world → continent → country drill-down to see where your guests come from. Pro adds provinces and city insight panels.</p><button type="button" onClick={() => { setModalTargetTier('growth'); setModalFeatureName(t('reports_visitor_origin_explorer')); setShowUpgradeModal(true); }} className="inline-flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl text-sm font-bold"><Zap size={14} />{t('reports_upgrade_to_growth')}</button></div><UpgradePromptModal isOpen={showUpgradeModal} onClose={() => setShowUpgradeModal(false)} currentTier={limits.subscriptionTier} targetTier={modalTargetTier} featureName={modalFeatureName} onUpgrade={() => { setShowUpgradeModal(false); window.location.href='/business/billing'; }} onCompare={() => { setShowUpgradeModal(false); window.location.href='/business/billing'; }} /></div>;
  ...
  return <div className="bg-white rounded-3xl shadow-xl border border-stone-200 overflow-hidden transition-all duration-300">
    <div className="sticky top-0 z-40 px-6 py-5 border-b border-stone-100 bg-white/95 backdrop-blur-md flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><div className="p-2 bg-orange-500 rounded-2xl text-white"><Globe2 size={22}/></div><div><h3 className="text-lg font-extrabold text-stone-900">{t('reports_visitor_origin_explorer')}</h3><p className="text-xs text-stone-400 mt-0.5">{t('reports_guest_checkins_summary', { count: totalVisitors, domestic: domesticCount, international: internationalCount })}</p></div></div></div>
    <div className="bg-stone-100/40 px-6 py-2 border-b border-stone-100 flex flex-wrap items-center gap-2 text-xs font-mono text-stone-400"><button type="button" className={currentLevel==='world'?'text-orange-500 font-bold':'hover:text-stone-600'} onClick={()=>jumpTo('world')}>World</button>{selectedContinent&&<><span>&gt;</span><button type="button" className={currentLevel==='countries'?'text-orange-500 font-bold':'hover:text-stone-600'} onClick={()=>jumpTo('countries')}>{selectedContinent}</button></>}{selectedCountry&&<><span>&gt;</span><button type="button" className={currentLevel==='regions'?'text-orange-500 font-bold':'hover:text-stone-600'} onClick={()=>jumpTo('regions')}>{selectedCountry}</button></>}{selectedRegion&&<><span>&gt;</span><span className={currentLevel==='cities'?'text-orange-500 font-bold':''}>{selectedRegion}</span></>}{selectedCity&&<><span>&gt;</span><span className="text-orange-500 font-bold">{selectedCity}</span></>}</div>
    <div className="p-6">{fetchError&&<div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{fetchError}</div>}{qualityNote&&!fetchError&&<div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">{qualityNote}</div>}
      {useGeoMap && <div className={currentLevel==='cityDetail'?'hidden':''}><div className="h-[380px] md:h-[440px]"><GeographicMapViewport level={geoLevel} nodes={geoNodes} selectedContinent={selectedContinent} selectedCountry={selectedCountry} selectedRegion={selectedRegion} isLoading={isBusy} interactive={interactive} onContinentClick={handleContinentClick} onCountryClick={handleCountryClick} onRegionClick={handleRegionClick} onCityClick={handleCityClick} onBack={handleBack} onHome={handleHome} continentOfCountry={continentOfCountryName}/></div></div>}
      {!useGeoMap && <>{currentLevel==='world'&&<VisitorOriginWorldMap totalVisitors={totalVisitors} onExplore={handleWorldExplore} isLoading={isBusy}/>} {currentLevel==='continents'&&<VisitorOriginContinentMap data={continentData} onContinentClick={handleContinentClick} onBack={handleBack} isLoading={isBusy}/>} {currentLevel==='countries'&&<VisitorOriginCountryMap data={nodes.map(n=>({country:n.name,continent:selectedContinent||n.continent||'',count:n.count,percentage:n.percentage}))} continentName={selectedContinent||''} onCountryClick={handleCountryClick} onBack={handleBack} isLoading={isBusy}/>} {currentLevel==='regions'&&<VisitorOriginRegionMap data={nodes.map(n=>({region:n.name,country:selectedCountry||'',count:n.count,percentage:n.percentage}))} countryName={selectedCountry||''} onRegionClick={handleRegionClick} onBack={handleBack} isLoading={isBusy}/>} {currentLevel==='cities'&&<VisitorOriginCityGrid data={nodes.map(n=>({city:n.name,region:selectedRegion||'',count:n.count,percentage:n.percentage}))} regionName={selectedRegion||selectedCountry||''} onBack={handleBack} isLoading={isBusy} onCityClick={handleCityClick}/>}</>}
      {currentLevel==='cityDetail'&&selectedCity&&<CityInsightPanel cityName={selectedCity} regionName={selectedRegion} countryName={selectedCountry} data={cityPanel} isLoading={isBusy} onBack={handleBack}/>}</div>
