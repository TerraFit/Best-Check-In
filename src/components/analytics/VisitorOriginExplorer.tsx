...
export function VisitorOriginExplorer({ businessId, dateFrom, dateTo, limits, canInteractiveMap = true, isLoading: parentLoading = false }: VisitorOriginExplorerProps) {
  ...
  const handleRegionClick = (region: string) => { if (!limits.canViewCities) { setModalTargetTier('pro'); setModalFeatureName('City Insights'); setShowUpgradeModal(true); return; } setSelectedRegion(region); setSelectedCity(null); if (isUnitedKingdom(selectedCountry) && regionNamesEqual(region, 'England')) { setCurrentLevel('ukEngland'); loadLevel('ukEngland', { continent: selectedContinent, country: selectedCountry, region: 'England' }); return; } setCurrentLevel('cities'); const apiRegion = isUnitedKingdom(selectedCountry) && isUkEnglandItl1Region(region) ? 'England' : region; loadLevel('cities', { continent: selectedContinent, country: selectedCountry, region: apiRegion }); };
