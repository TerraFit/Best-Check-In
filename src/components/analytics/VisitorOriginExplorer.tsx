...
function toApiLevel(uiLevel: UiLevel): DrillLevel { if (uiLevel === 'world') return 'world'; if (uiLevel === 'continents') return 'continent'; if (uiLevel === 'countries') return 'country'; if (uiLevel === 'regions') return 'region'; return 'city'; }
...
const COUNTRY_TO_CONTINENT: Record<string, string> = {
  ...
  'United States': 'North America', Canada: 'North America', Mexico: 'North America', Brazil: 'South America', Argentina: 'South America', Chile: 'South America', Colombia: 'South America', Peru: 'South America',
  Australia: 'Oceania', 'New Zealand': 'Oceania', Fiji: 'Oceania', China: 'Asia', India: 'Asia', Japan: 'Asia', 'South Korea': 'Asia', Singapore: 'Asia', Malaysia: 'Asia', Indonesia: 'Asia', Thailand: 'Asia', Vietnam: 'Asia', Philippines: 'Asia', 'United Arab Emirates': 'Asia', 'Saudi Arabia': 'Asia', Israel: 'Asia', Pakistan: 'Asia', Bangladesh: 'Asia',
};
...
export function VisitorOriginExplorer({ businessId, dateFrom, dateTo, limits, canInteractiveMap = true, isLoading: parentLoading = false }: VisitorOriginExplorerProps) {
  ...
  const loadLevel = useCallback(async (uiLevel: UiLevel, parent: { continent?: string | null; country?: string | null; region?: string | null; city?: string | null }) => {
    ...
    try {
      ...
      if (res.upgradeRequired) { setModalTargetTier((res.requiredPlan as SubscriptionTier) || 'growth'); setModalFeatureName(res.error || 'Upgrade required'); setShowUpgradeModal(true); setLoading(false); return; }
      if (!res.success) { setFetchError(res.error || 'Failed to load origins'); setNodes([]); setQualityNote(null); }
      else {
  ...
  const handleWorldExplore = () => { if (!interactive) { setModalTargetTier('growth'); setModalFeatureName(t('reports_visitor_origin_explorer')); setShowUpgradeModal(true); return; } setCurrentLevel('continents'); loadLevel('continents', {}); };
  ...
  const handleBack = () => {
    if (currentLevel === 'cityDetail') { setSelectedCity(null); setCurrentLevel('cities'); loadLevel('cities', { continent: selectedContinent, country: selectedCountry, region: selectedRegion }); }
    else if (currentLevel === 'cities') { setSelectedRegion(null); if (selectedCountry && limits.canViewRegions) { setCurrentLevel('regions'); loadLevel('regions', { continent: selectedContinent, country: selectedCountry }); } else { setCurrentLevel('countries'); loadLevel('countries', { continent: selectedContinent }); } }
    else if (currentLevel === 'regions') { setSelectedCountry(null); setCurrentLevel('countries'); loadLevel('countries', { continent: selectedContinent }); }
    else if (currentLevel === 'countries') { setSelectedContinent(null); setCurrentLevel('continents'); loadLevel('continents', {}); }
    else if (currentLevel === 'continents') { setCurrentLevel('world'); loadLevel('world', {}); }
  };
  ...
  const jumpTo = (target: UiLevel) => {
    ...
    if (target === 'continents') { setSelectedContinent(null); setSelectedCountry(null); setSelectedRegion(null); setSelectedCity(null); setCurrentLevel('continents'); loadLevel('continents', {}); return; }
    if (target === 'countries' && selectedContinent) { setSelectedCountry(null); setSelectedRegion(null); setSelectedCity(null); setCurrentLevel('countries'); loadLevel('countries', { continent: selectedContinent }); return; }
    if (target === 'regions' && selectedCountry) { setSelectedRegion(null); setSelectedCity(null); setCurrentLevel('regions'); loadLevel('regions', { continent: selectedContinent, country: selectedCountry }); return; }
    if (target === 'cities' && selectedRegion) { setSelectedCity(null); setCurrentLevel('cities'); loadLevel('cities', { continent: selectedContinent, country: selectedCountry, region: selectedRegion }); }
  };
  ...
  const gridLevel = currentLevel === 'world' ? 'continents' : currentLevel === 'continents' ? 'continents' : currentLevel === 'countries' ? 'regions' : 'cities';
  const gridTitle = currentLevel === 'world' ? 'Bookings by continent' : currentLevel === 'continents' ? 'Bookings by continent' : currentLevel === 'countries' ? `Bookings by province / state / region · ${selectedCountry || selectedContinent || 'World'}` : `Bookings by city · ${selectedRegion || selectedCountry || ''}`;
  const gridSubtitle = currentLevel === 'world' ? 'Select a continent to drill into its countries.' : currentLevel === 'continents' ? 'Select a continent to see its countries.' : currentLevel === 'countries' ? 'Select an administrative area to see its cities.' : 'Select a city to open the detailed visitor insights.';
  const gridSelect = currentLevel === 'world' || currentLevel === 'continents' ? handleContinentClick : currentLevel === 'countries' ? handleRegionClick : handleCityClick;
  const contributionGrid = <VisitorOriginContributionGrid level={gridLevel} nodes={geoNodes} title={gridTitle} subtitle={gridSubtitle} onBack={!useGeoMap && currentLevel !== 'world' ? handleBack : undefined} onSelect={gridSelect} />;
  const geoContributionGrid = <VisitorOriginContributionGrid level={gridLevel} nodes={geoNodes} title={gridTitle} subtitle={gridSubtitle} onSelect={gridSelect} overlay />;
      ...
      <div className="p-6 space-y-6">
        ...
        {useGeoMap && currentLevel !== 'cityDetail' ? (
          <div className="relative h-[440px] md:h-[500px]">
            <GeographicMapViewport level={geoLevel} nodes={geoNodes} selectedContinent={selectedContinent} selectedCountry={selectedCountry} selectedRegion={selectedRegion} isLoading={isBusy} interactive={interactive} onContinentClick={handleContinentClick} onCountryClick={handleCountryClick} onRegionClick={handleRegionClick} onCityClick={handleCityClick} onBack={handleBack} onHome={handleHome} continentOfCountry={continentOfCountryName} overlayInset={320} />
            <div className="absolute left-3 top-3 z-30 hidden w-[320px] md:block">{geoContributionGrid}</div>
          </div>
        ) : (
          <>{currentLevel === 'world' && <VisitorOriginWorldMap totalVisitors={totalVisitors} onExplore={handleWorldExplore} isLoading={isBusy} />}{currentLevel === 'continents' && <VisitorOriginContinentMap data={continentData} onContinentClick={handleContinentClick} onBack={handleBack} isLoading={isBusy} />}{currentLevel === 'countries' && <VisitorOriginCountryMap data={nodes.map(n => ({ country: n.name, continent: selectedContinent || n.continent || '', count: n.count, percentage: n.percentage }))} continentName={selectedContinent || ''} onCountryClick={handleCountryClick} onBack={handleBack} isLoading={isBusy} />}{currentLevel === 'regions' && <VisitorOriginRegionMap data={nodes.map(n => ({ region: n.name, country: selectedCountry || '', count: n.count, percentage: n.percentage }))} countryName={selectedCountry || ''} onRegionClick={handleRegionClick} onBack={handleBack} isLoading={isBusy} />}{currentLevel === 'cities' && <VisitorOriginCityGrid data={nodes.map(n => ({ city: n.name, region: selectedRegion || '', count: n.count, percentage: n.percentage }))} regionName={selectedRegion || selectedCountry || ''} onBack={handleBack} isLoading={isBusy} onCityClick={handleCityClick} />}</>
        )}
        {useGeoMap && currentLevel !== 'cityDetail' && <div className="md:hidden">{contributionGrid}</div>}
        {!useGeoMap && currentLevel !== 'cityDetail' && contributionGrid}
