...
export function VisitorOriginExplorer({ businessId, dateFrom, dateTo, limits, canInteractiveMap = true, isLoading: parentLoading = false }: VisitorOriginExplorerProps) {
  ...
  const handleRegionClick = (region: string) => { if (!limits.canViewCities) { setModalTargetTier('pro'); setModalFeatureName('City Insights'); setShowUpgradeModal(true); return; } setSelectedRegion(region); setSelectedCity(null); if (selectedCountry === 'United Kingdom' && regionNamesEqual(region, 'England')) { setCurrentLevel('regions'); loadLevel('regions', { continent: selectedContinent, country: selectedCountry, region: 'England' }); return; } setCurrentLevel('cities'); const apiRegion = selectedCountry === 'United Kingdom' && isUkEnglandItl1Region(region) ? 'England' : region; loadLevel('cities', { continent: selectedContinent, country: selectedCountry, region: apiRegion }); };
