...
type UiLevel = 'world' | 'countries' | 'regions' | 'ukEngland' | 'cities' | 'cityDetail';
type ParentSelection = { continent?: string | null; country?: string | null; region?: string | null; city?: string | null };
function toApiLevel(uiLevel: UiLevel): DrillLevel { if (uiLevel === 'world') return 'world'; if (uiLevel === 'countries') return 'country'; if (uiLevel === 'regions' || uiLevel === 'ukEngland') return 'region'; return 'city'; }
...
export function VisitorOriginExplorer({ businessId, dateFrom, dateTo, limits, canInteractiveMap = true, isLoading: parentLoading = false }: VisitorOriginExplorerProps) {
  ...
  const handleRegionClick = (region: string) => { if (!limits.canViewCities) { setModalTargetTier('pro'); setModalFeatureName('City Insights'); setShowUpgradeModal(true); return; } setSelectedRegion(region); setSelectedCity(null); const ukEngland = isUnitedKingdom(selectedCountry) && regionNamesEqual(region, 'England'); if (ukEngland) { setCurrentLevel('ukEngland'); loadLevel('ukEngland', { continent: selectedContinent, country: selectedCountry, region: 'England' }); return; } setCurrentLevel('cities'); const apiRegion = isUnitedKingdom(selectedCountry) && isUkEnglandItl1Region(region) ? 'England' : region; loadLevel('cities', { continent: selectedContinent, country: selectedCountry, region: apiRegion }); };
  ...
  const handleBack = () => {
    ...
    if (currentLevel === 'ukEngland') { setCurrentLevel('regions'); loadLevel('regions', { continent: selectedContinent, country: selectedCountry }); return; }
