...
export function VisitorOriginCityGrid({
  ...
  const cityList = useMemo(() => {
    ...
    data.forEach((item) => {
      const city = item.city || item.town || item.name || item.key;
      if (city) {
  ...
  const total = useMemo(() => cityList.reduce((sum, c) => sum + c.count, 0), [cityList]);
      ...
      <div className="space-y-4 max-w-2xl mx-auto">
        {cityList.map((city, index) => {
          ...
          return (
            <div
              key={city.name}
              className={`relative ${onCityClick ? 'cursor-pointer' : ''}`}
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-8 flex items-center justify-center">
                  {index < 3 ? (
                    ...
                    <span className="text-xs font-bold text-stone-400 font-mono">#{index + 1}</span>
                  )}
                </div>
                ...
                <div className="w-28 text-right truncate">
                  <span
                    className={`text-xs font-semibold ${
                      isHovered ? 'text-orange-600 font-bold' : 'text-stone-700'
                    }`}
                  >
                    {city.name}
                  </span>
                ...
                <div className="flex-1 h-7 bg-stone-200/60 rounded-lg overflow-hidden shadow-inner relative flex items-center">
                  ...
                  >
                    <span className="text-[10px] font-extrabold text-white font-mono select-none">
                      {city.count}
                    </span>
                  </div>
                </div>
              ...
              {isHovered && (
                <div className="absolute left-1/2 -top-10 transform -translate-x-1/2 bg-stone-900 text-white px-3 py-1.5 rounded-lg shadow-xl text-[11px] font-medium border border-stone-800 z-10 whitespace-nowrap pointer-events-none">
                  {onCityClick ? 'Click for city insights · ' : ''}
                  <strong className="text-orange-400">{city.name}</strong> · {city.count} visitors
                </div>
              )}
            </div>
          );
...
export default VisitorOriginCityGrid;
