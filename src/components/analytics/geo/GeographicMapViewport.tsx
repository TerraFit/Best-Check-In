...
const SOURCE_ID = 'analytics-geo'; const FILL_LAYER = 'analytics-geo-fill'; const LINE_LAYER = 'analytics-geo-line'; const CITY_LAYER = 'analytics-geo-cities'; const CITY_LABEL_LAYER = 'analytics-geo-city-labels';
const CONTINENT_BOUNDS: Record<string, [number, number, number, number]> = {
  Africa: [-20, -36, 53, 38], Europe: [-12, 34, 43, 72], 'North America': [-170, 5, -50, 75], 'South America': [-86, -57, -30, 14], Asia: [25, -10, 180, 80], Oceania: [105, -50, 180, 5], Other: [-180, -60, 180, 80],
};
const normalizeContinent = (value: unknown): string => {
  if (typeof value !== 'string') return 'Other';
  const key = value.trim().toLowerCase();
  const match = Object.keys(CONTINENT_BOUNDS).find(name => name.toLowerCase() === key);
  return match || 'Other';
};
...
function featureContinent(feature: FeatureLike, getContinent: (country: string) => string): string {
  const props = feature.properties || {};
  const direct = props.continent ?? props.CONTINENT ?? props.continent_name ?? props.CONTINENT_NAME;
  const normalizedDirect = normalizeContinent(direct);
  if (normalizedDirect !== 'Other') return normalizedDirect;
  return normalizeContinent(getContinent(featureCountryName(props, feature.id)));
}
...
function GeographicMapViewportInner({ level, nodes, selectedContinent, selectedCountry, selectedRegion, isLoading = false, interactive = true, onContinentClick, onCountryClick, onRegionClick, onCityClick, onBack, onHome, continentOfCountry, overlayInset = 320 }: GeographicMapViewportProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const regionLevel = level === 'regions' && !!selectedCountry; const fc = regionLevel ? await loadAdmin1() : await loadCountries50m().catch(() => loadCountries110m()); if (cancelled) return;
      ...
      if (regionLevel) {
        ...
        features = fc.features.map((feature, index) => { const name = featureCountryName(feature.properties || {}, feature.id ?? feature.properties?.id as string | number | undefined); const continent = featureContinent(feature, getContinent); const node = level === 'world' ? nodeByContinent.get(continent.toLowerCase()) || null : findNodeForFeature(name, nodes, feature.id) || nodeByCountry.get(canonicalCountryName(name).toLowerCase()) || null; const count = node?.count ?? 0; return { ...feature, id: feature.id ?? index, properties: { ...feature.properties, name, continent, count, percentage: node?.percentage ?? 0, hasGuests: count > 0, fillColor: heatColor(count), isSelected: !!selectedCountry && canonicalCountryName(name).toLowerCase() === canonicalCountryName(selectedCountry).toLowerCase() } }; });
      }
      map.getSource(SOURCE_ID)?.setData?.({ type: 'FeatureCollection', features });
      if (!preserveCamera) {
  ...
  return (
    <div className="relative w-full h-full min-h-[320px] overflow-hidden rounded-xl border border-stone-200 bg-slate-50">
      ...
      {interactive && (onBack || onHome) && (
        <div className="absolute right-3 top-3 z-40 flex items-center gap-1 rounded-xl border border-stone-200 bg-white/95 p-1.5 shadow-lg backdrop-blur-sm">
