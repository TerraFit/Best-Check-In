...
function GeographicMapViewportInner({
        ...
        map.on('load', async () => {
          ...
          try {
            ...
            map.addLayer({ id: CITY_LAYER, type: 'circle', source: SOURCE_ID, layout: { visibility: 'none' }, paint: {
              ...
              'circle-color': ['get', 'fillColor'], 'circle-stroke-color': '#ffffff', 'circle-stroke-width': 2, 'circle-opacity': 0.95,
            } });
            map.addLayer({ id: CITY_LABEL_LAYER, type: 'symbol', source: SOURCE_ID, layout: {
              visibility: 'none', 'text-field': ['get', 'name'], 'text-size': 11, 'text-offset': [0, 1.25], 'text-anchor': 'top', 'text-allow-overlap': false,
    ...
    (async () => {
      ...
      if (cityLevel) {
        ...
        try {
          ...
          const features = points.map((point, index) => ({
            ...
            properties: { name: point.name, count: point.count, percentage: point.percentage, hasGuests: point.count > 0, fillColor: heatColor(point.count) },
          }));
          ...
          if (points.length === 1) {
            ...
            const matching = admin1.features.filter((feature) => {
              ...
              return canonicalCountryName(country).toLowerCase() === canonicalCountryName(selectedCountry || '').toLowerCase() && featureRegionName(feature.properties || {}).toLowerCase() === selectedRegion.toLowerCase();
            });
      ...
      if (regionLevel) {
        ...
        features = fc.features.map((feature, index) => {
          ...
          const count = node?.count ?? 0;
          return { ...feature, id: feature.id ?? index, properties: { ...feature.properties, name, count, percentage: node?.percentage ?? 0, hasGuests: count > 0, fillColor: heatColor(count), isSelected: !!selectedCountry && canonical === canonicalCountryName(selectedCountry).toLowerCase() } };
        });
      }
  ...
  useEffect(() => {
    ...
    for (const layer of [FILL_LAYER, CITY_LAYER]) {
    ...
    return () => {
      try {
        for (const layer of [FILL_LAYER, CITY_LAYER]) {
          map.off('click', layer, onClick); map.off('mousemove', layer, onMove); map.off('mouseleave', layer, onLeave);
        }
      } catch { /* ignore */ }
    };
    ...
    <div className="relative w-full h-full min-h-[320px] rounded-xl overflow-hidden bg-slate-50 border border-stone-200">
      ...
      {interactive && level !== 'world' && (onBack || onHome) && (
        <div className="absolute top-3 left-3 z-40 flex items-center gap-1.5 rounded-xl bg-white/95 border border-stone-200 shadow-lg p-1.5 backdrop-blur-sm">
          {onBack && <button type="button" onClick={onBack} className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-bold text-stone-700 hover:bg-stone-100" aria-label="Go back">← Back</button>}
          {onHome && <button type="button" onClick={onHome} className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-bold text-stone-600 hover:bg-stone-100" aria-label="Return to world">World</button>}
        </div>
      )}

      {cityNodes.length > 0 && <div className="absolute top-16 left-3 z-30 w-[min(300px,calc(100%-24px))] max-h-[calc(100%-88px)] overflow-y-auto rounded-xl bg-white/95 border border-stone-200 shadow-lg backdrop-blur-sm">
        <div className="px-3 py-2 border-b border-stone-200"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Cities on map</p><p className="text-[11px] text-stone-400 mt-0.5">Click a marker or city name</p></div>
        <div className="p-1.5">{cityNodes.map((node) => <button key={`${node.name}-${node.code || ''}`} type="button" disabled={!interactive || node.count <= 0} onClick={() => onCityClick?.(node.name)} className={`w-full flex items-center gap-2 rounded-lg px-2.5 py-2 text-left transition ${node.count <= 0 ? 'opacity-45 cursor-default' : 'cursor-pointer hover:bg-stone-100'}`}><span className="h-3 w-3 rounded-full shrink-0 border border-white shadow-sm" style={{ backgroundColor: node.count > 0 ? heatColor(node.count) : '#e5e7eb' }} /><span className="min-w-0 flex-1 truncate text-xs font-medium text-stone-700">{node.name}</span><span className="text-xs font-bold text-stone-500 tabular-nums">{node.count}</span></button>)}</div>
      </div>}
