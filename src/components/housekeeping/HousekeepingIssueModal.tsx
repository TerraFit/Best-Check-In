...
export default function HousekeepingIssueModal({ businessId, taskId, roomId, roomNumber, sessionId, itemId, itemLabel, onClose, onCreated }: Props) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-lg max-h-[92vh] overflow-y-auto rounded-3xl bg-white shadow-2xl">
    <div className="px-5 py-4 border-b border-gray-200 flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-orange-600">Report Issue</p><h2 className="text-lg font-bold text-gray-900">{itemLabel}</h2><p className="text-xs text-gray-500 mt-1">{option.category}</p></div><button type="button" onClick={onClose} className="w-10 h-10 rounded-full bg-gray-100 text-gray-600 text-xl" aria-label={t('common_close')}>×</button></div>
    <div className="p-5 space-y-4">{error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
      ...
      <div><span className="block text-sm font-semibold text-gray-800 mb-2">{t('housekeeping_priority')}</span><div className="grid grid-cols-4 gap-2">{(['low','medium','high','urgent'] as const).map((value) => <button type="button" key={value} aria-pressed={priority === value} onClick={() => setPriority(value)} className={`rounded-xl border-2 py-2.5 text-xs font-bold capitalize ${priority === value ? 'bg-orange-500 border-orange-500 text-white' : 'border-gray-200 text-gray-600 bg-white'}`}>{value}</button>)}</div></div>
    ...
    <div className="px-5 py-4 border-t border-gray-200 flex justify-end gap-2"><button type="button" onClick={onClose} disabled={saving || uploadingPhoto} className="px-4 py-2.5 rounded-xl border border-gray-300 text-sm font-semibold text-gray-700">{t('common_cancel')}</button><button type="button" onClick={() => void submit()} disabled={saving || uploadingPhoto} className="px-5 py-2.5 rounded-xl bg-orange-500 text-white text-sm font-bold disabled:opacity-50">{saving ? 'Saving…' : t('common_save')}</button></div>
  </div></div>;
}
