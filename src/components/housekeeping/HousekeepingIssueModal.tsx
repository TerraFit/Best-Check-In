...
export default function HousekeepingIssueModal({ businessId, taskId, roomId, roomNumber, sessionId, itemId, itemLabel, onClose, onCreated }: Props) {
  ...
  const submit = async () => {
    if (issueType === 'Other' && !otherDescription.trim()) { setError('Please describe the issue when selecting Other.'); return; }
    setSaving(true); setError(null);
    try {
      const issue = await createHousekeepingIssue({ businessId, taskId, roomId, roomNumber: roomNumber || undefined, sessionId, checklistItemId: itemId, checklistItemLabel: itemLabel, category: option.category, issueType, otherDescription: issueType === 'Other' ? otherDescription.trim() : undefined, description: description.trim() || undefined, priority, maintenanceRequested, photoUrl: photoUrl.trim() || undefined });
      onCreated(issue);
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to save the issue.'); } finally { setSaving(false); }
  };

  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden">
