...
import { createHousekeepingIssue } from '../../services/housekeepingApi';
...
interface Props {
  ...
  itemLabel: string;
  onClose: () => void;
  onCreated: (issue: HousekeepingIssue) => void;
}

export default function HousekeepingIssueModal({ businessId, taskId, roomId, sessionId, itemId, itemLabel, onClose, onCreated }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      const issue = await createHousekeepingIssue({ businessId, taskId, roomId, sessionId, checklistItemId: itemId, checklistItemLabel: itemLabel, category: option.category, issueType, otherDescription: issueType === 'Other' ? otherDescription.trim() : undefined, description: description.trim() || undefined, priority, maintenanceRequested, photoUrl: photoUrl.trim() || undefined });
      onCreated(issue);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unable to save the issue.');
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4">
    <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-200 flex items-start justify-between gap-4">
        <div><p className="text-xs font-bold uppercase tracking-wider text-orange-600">Report Issue</p><h2 className="text-lg font-bold text-gray-900">{itemLabel}</h2><p className="text-xs text-gray-500 mt-1">{option.category}</p></div>
        <button type="button" onClick={onClose} className="w-10 h-10 rounded-full bg-gray-100 text-gray-600 text-xl" aria-label="Close">×</button>
      </div>
      <div className="p-5 space-y-4">
        ...
        <label className="flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50 p-3 cursor-pointer"><input type="checkbox" checked={maintenanceRequested} onChange={(e) => setMaintenanceRequested(e.target.checked)} className="mt-1 h-4 w-4 rounded border-gray-300 text-blue-600" /><span><span className="block text-sm font-semibold text-blue-900">Send to Maintenance</span><span className="block text-xs text-blue-700 mt-0.5">Create the issue as a Maintenance follow-up for management.</span></span></label>
        <label className="block"><span className="block text-sm font-semibold text-gray-800 mb-1">Photo URL <span className="font-normal text-gray-400">(optional)</span></span><input value={photoUrl} onChange={(e) => setPhotoUrl(e.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm" placeholder="Optional uploaded photo URL" /></label>
      </div>
      <div className="px-5 py-4 border-t border-gray-200 flex justify-end gap-2"><button type="button" onClick={onClose} disabled={saving} className="px-4 py-2.5 rounded-xl border border-gray-300 text-sm font-semibold text-gray-700">Cancel</button><button type="button" onClick={() => void submit()} disabled={saving} className="px-5 py-2.5 rounded-xl bg-orange-500 text-white text-sm font-bold disabled:opacity-50">{saving ? 'Saving…' : 'Save Issue'}</button></div>
    </div>
  </div>;
}
