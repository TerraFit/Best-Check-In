...
export default function HousekeepingServiceModal({ businessId, task, session, onClose, onCompleted }: Props) {
  ...
  const toggleItem = (id: string) => { const next = { ...checked, [id]: !checked[id] }; setChecked(next); void persist(next); };
  const reportIssue = (label: string) => { const description = window.prompt(`Report issue found: ${label}`); if (!description?.trim()) return; const nextIssueCount = issueCount + 1; const nextNotes = `${notes ? `${notes}\n` : ''}Issue: ${label} — ${description.trim()}`; setIssueCount(nextIssueCount); setNotes(nextNotes); void persist(checked, nextIssueCount, nextNotes); };
  ...
  return <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4"><div className="w-full sm:max-w-3xl max-h-[95vh] overflow-hidden bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col">
    <div className="px-5 py-4 border-b border-gray-200 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-orange-600">Housekeeping Service</p><h2 className="text-xl font-bold text-gray-900">Room {task.room_number ?? '—'}{task.room_name ? ` · ${task.room_name}` : ''}</h2><p className="text-sm text-gray-500">{task.task_type === 'full_service' ? 'Full Service' : 'Refresh'}{task.room_type ? ` · ${task.room_type}` : ''}</p></div><button type="button" onClick={onClose} className="w-11 h-11 rounded-full bg-gray-100 text-gray-600 text-xl" aria-label="Close">×</button></div>
    ...
    <div className="px-5 py-4 overflow-y-auto space-y-5">{checklist.map((section) => { const sectionDone = section.items.filter((item) => checked[item.id]).length; return <section key={section.id} className="rounded-2xl border border-gray-200 overflow-hidden"><div className="px-4 py-3 bg-gray-50 flex items-center justify-between"><h3 className="font-bold text-gray-900">{section.title}</h3><span className="text-xs font-semibold text-gray-500">{sectionDone} / {section.items.length}</span></div><div className="divide-y divide-gray-100">{section.items.map((item) => <div key={item.id} className="px-4 py-3 flex items-center gap-3 min-h-[58px]"><button type="button" onClick={() => toggleItem(item.id)} className={`shrink-0 w-8 h-8 rounded-lg border-2 flex items-center justify-center text-lg ${checked[item.id] ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-gray-300 text-transparent'}`}>✓</button><span className={`flex-1 text-sm ${checked[item.id] ? 'text-gray-400 line-through' : 'text-gray-800'}`}>{item.label}</span>{item.issueReportable && <button type="button" onClick={() => reportIssue(item.label)} className="shrink-0 px-2.5 py-1.5 text-[11px] font-semibold rounded-lg border border-orange-200 text-orange-700 bg-orange-50">Report issue</button>}</div>)}</div></section>; })}{issueCount > 0 && <div className="rounded-xl bg-orange-50 border border-orange-200 px-4 py-3 text-sm text-orange-800"><strong>{issueCount}</strong> issue{issueCount === 1 ? '' : 's'} recorded during this service.</div>}{saving && <p className="text-xs text-gray-400 text-center">Saving service progress…</p>}</div>
