...
export default function HousekeepingServiceModal({ businessId, task, session, onClose, onCompleted }: Props) {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setElapsed(Math.max(0, Math.floor((Date.now() - new Date(startedAt).getTime()) / 1000))), 250);
  ...
  const persist = async (next: Record<string, boolean>, nextIssueCount = issueCount, notes?: string) => {
    setSaving(true);
    try {
      await updateHousekeepingServiceProgress({
        ...
        checklistState: next,
        checklistCompletedCount: itemIds.filter((id) => next[id]).length,
        checklistTotalCount: itemIds.length,
  ...
  const reportIssue = (label: string) => {
    ...
    void persist(checked, nextIssueCount, `${task.notes ? `${task.notes}\n` : ''}Issue: ${label} — ${description.trim()}`);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
      <div className="w-full sm:max-w-3xl max-h-[95vh] overflow-hidden bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col">
        <div className="px-5 py-4 border-b border-gray-200 flex items-start justify-between gap-4">
          <div><p className="text-xs font-semibold uppercase tracking-wider text-orange-600">Housekeeping Service</p><h2 className="text-xl font-bold text-gray-900">Room {task.room_number ?? '—'}{task.room_name ? ` · ${task.room_name}` : ''}</h2><p className="text-sm text-gray-500">{task.task_type === 'full_service' ? 'Full Service' : 'Refresh'}{task.room_type ? ` · ${task.room_type}` : ''}</p></div>
          <button type="button" onClick={onClose} className="w-11 h-11 rounded-full bg-gray-100 text-gray-600 text-xl" aria-label="Close">×</button>
        </div>

        <div className="px-5 pt-4"><div className={`rounded-2xl border-2 p-4 ${timerClass}`}>
          <div className="flex items-center justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-wider opacity-70">{overTarget ? 'OVER TARGET' : finalCountdown ? 'FINAL COUNTDOWN' : warning ? 'WARNING' : 'TIME REMAINING'}</p><p className="text-4xl sm:text-5xl font-black tabular-nums leading-none mt-1">{overTarget ? `+${formatDuration(elapsed - targetSeconds)}` : formatDuration(remaining)}</p></div><div className="text-right text-xs font-medium"><p>Target</p><p className="text-base font-bold tabular-nums">{formatDuration(targetSeconds)}</p><p className="mt-2">Elapsed {formatDuration(elapsed)}</p></div></div>
          <div className="mt-3 flex items-center justify-between gap-3 text-xs"><span>{progressCount} / {itemIds.length} checklist items</span><button type="button" onClick={() => setMuted((value) => !value)} className="px-3 py-1.5 rounded-full bg-white/80 border border-current/20 font-semibold">{muted ? '🔇 Muted' : '🔊 Voice & sound'}</button></div>
        </div></div>

        <div className="px-5 py-4 overflow-y-auto space-y-5">
          {checklist.map((section) => { const sectionDone = section.items.filter((item) => checked[item.id]).length; return <section key={section.id} className="rounded-2xl border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 bg-gray-50 flex items-center justify-between"><h3 className="font-bold text-gray-900">{section.title}</h3><span className="text-xs font-semibold text-gray-500">{sectionDone} / {section.items.length}</span></div>
            <div className="divide-y divide-gray-100">{section.items.map((item) => <div key={item.id} className="px-4 py-3 flex items-center gap-3 min-h-[58px]">
              ...
              {item.issueReportable && <button type="button" onClick={() => reportIssue(item.label)} className="shrink-0 px-2.5 py-1.5 text-[11px] font-semibold rounded-lg border border-orange-200 text-orange-700 bg-orange-50">Report issue</button>}
            </div>)}</div>
          </section>; })}
