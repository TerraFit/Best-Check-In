...
export default function HousekeepingServiceModal({ businessId, task, session, onClose, onCompleted }: Props) {
  ...
  const progressCount = itemIds.filter((id) => !TEMPERATURE_IDS.includes(id) && checked[id]).length + (hasTemperatureChoice && temperatureSelected ? 1 : 0);
  ...
  return <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4"><div className="w-full sm:max-w-3xl max-h-[95vh] overflow-hidden bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col">
    ...
    <div className="px-5 py-4 overflow-y-auto space-y-5">
      {checklist.map((section) => {
        ...
        const visibleItems = section.items.filter((item) => !TEMPERATURE_IDS.includes(item.id) || item.id === 'full-temperature');
        const sectionDone = section.items.filter((item) => !TEMPERATURE_IDS.includes(item.id) && checked[item.id]).length + (sectionTemperature && temperatureSelected ? 1 : 0);
        return <section key={section.id} className="rounded-2xl border border-gray-200 overflow-hidden"><div className="px-4 py-3 bg-gray-50 flex items-center justify-between"><h3 className="font-bold text-gray-900">{section.title}</h3><span className="text-xs font-semibold text-gray-500">{sectionDone} / {visibleItems.length}</span></div><div className="divide-y divide-gray-100">{visibleItems.map((item) => {
          ...
          if (isTemperature) {
            return <div key={item.id} className="px-4 py-3 space-y-3"><div className="text-sm font-semibold text-gray-800">Room temperature</div><div className="grid grid-cols-3 gap-2">{[
              ['full-temperature', 'Comfortable', 'comfortable'],
              ['full-too-cold', 'Too cold', 'too_cold'],
              ['full-too-hot', 'Too hot', 'too_hot'],
            ].map(([id, label, choice]) => <button key={id} type="button" aria-pressed={checked[id]} onClick={() => setTemperature(choice as 'comfortable' | 'too_cold' | 'too_hot')} className={`min-h-[48px] rounded-xl border-2 px-2 text-sm font-semibold flex items-center justify-center gap-2 ${checked[id] ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-gray-300 text-gray-700'}`}><span className={`w-6 h-6 rounded-md border-2 flex items-center justify-center ${checked[id] ? 'border-white bg-white text-green-700' : 'border-gray-400 text-transparent'}`}>✓</span>{label}</button>)}</div><button type="button" onClick={() => reportIssue('Room temperature')} className="px-2.5 py-1.5 text-[11px] font-semibold rounded-lg border border-orange-200 text-orange-700 bg-orange-50">Report issue</button></div>;
          }
