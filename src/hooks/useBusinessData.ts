...
export function useBusinessData(activeTab: string, currentPage: number, pageSize: number, currentFilters: any) {
  ...
  const loadBusinessProfile = useCallback(async (force = false) => {
    if (loadingBusinessRef.current && !force) return;
    if (initialLoadDoneRef.current && !force) return;
  ...
  const loadBookings = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === 'reports' || activeTab === 'checkins') {
        if (currentFilters?.startDate && currentFilters?.endDate) {
          ...
          if (days[currentFilters.dateRange]) {
            ...
            cutoff.setDate(cutoff.getDate() - days[currentFilters.dateRange]);
            url += `&startDate=${cutoff.toISOString().split('T')[0]}`;
          }
        }
      ...
      const rawBookings: Booking[] = Array.isArray(result.bookings)
        ? result.bookings
      ...
      const total = result.total_count ?? validBookings.length;
      setTotalBookingsCount(total);
      setTotalPages(activeTab === 'reports' ? 1 : Math.max(1, result.total_pages ?? Math.ceil(total / pageSize)));
      ...
      if (result.facets) {
        ...
        setUniqueProvinces([...new Set(validBookings.map(b => b.guest_province).filter(Boolean))].sort() as string[]);
        setUniqueCities([...new Set(validBookings.map(b => b.guest_city).filter(Boolean))].sort() as string[]);
        setUniqueCountries([...new Set(validBookings.map(b => b.guest_country?.replace(/\.$/, '').trim()).filter(Boolean))].sort() as string[]);
      }

      const todayStr = new Date().toISOString().split('T')[0];
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      setTodayArrivals(validBookings.filter(b => b.check_in_date === todayStr));
      setTodayCheckouts(validBookings.filter(b => b.check_out_date === todayStr));
      setTodayStayovers(validBookings.filter(b => {
        if (!b.check_in_date) return false;
        const checkIn = new Date(b.check_in_date); checkIn.setHours(0, 0, 0, 0);
        if (checkIn >= today) return false;
        if (!b.check_out_date) return true;
        const checkOut = new Date(b.check_out_date); checkOut.setHours(0, 0, 0, 0);
        return checkOut > today;
      }));

      console.log(`📦 Loaded ${validBookings.length} bookings (filtered total: ${total})`);
    } catch (err: any) {
      if (err.name !== 'AbortError' && isMountedRef.current) console.error('❌ Error loading bookings:', err);
    } finally {
  ...
  useEffect(() => {
    if (business && initialLoadDoneRef.current) loadBookings();
  }, [business, activeTab, currentPage, pageSize, currentFilters?.dateRange, currentFilters?.startDate, currentFilters?.endDate, currentFilters?.statusFilter, currentFilters?.provinceFilter, currentFilters?.cityFilter, currentFilters?.countryFilter, currentFilters?.searchTerm, loadBookings]);

  const refreshData = useCallback(() => {
    lastFiltersRef.current = '';
    void loadBusinessProfile(true);
    if (business) void loadBookings();
  }, [business, loadBookings, loadBusinessProfile]);
