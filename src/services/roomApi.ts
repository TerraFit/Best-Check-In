...
export async function syncRooms(payload: { businessId: string; totalRooms: number; confirmDeactivate?: boolean }): Promise<SyncRoomsResult> {
  const token = getAuthToken();
...
export async function updateRoom(roomId: string, businessId: string, updates: RoomUpdatePayload): Promise<Room> {
  const token = getAuthToken();
  const headers: Record<string, string> = { 'Content-Type': 'application/json' };
  if (token) headers.Authorization = `Bearer ${token}`;
  const res = await fetch('/.netlify/functions/update-room', {
    method: 'POST', headers, body: JSON.stringify({ roomId, businessId, ...updates }),
  });
  const data = await parseJson(res);
  return data.room || data.data;
}

export async function assignRoomToBooking(payload: AssignRoomPayload): Promise<{ success: boolean; booking?: any; room?: Room | null }> {
  const token = getAuthToken();
  const headers: Record<string, string> = { 'Content-Type': 'application/json' };
  if (token) headers.Authorization = `Bearer ${token}`;
  const res = await fetch('/.netlify/functions/assign-room-to-booking', { method: 'POST', headers, body: JSON.stringify(payload) });
  return parseJson(res);
}
