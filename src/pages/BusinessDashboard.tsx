...
export default function BusinessDashboard() {
  ...
  const updateFilter = useCallback((key: string, value: any) => { updateFilterBase(key as any, value); setCurrentPage(1); }, [updateFilterBase, setCurrentPage]);
  const { business, bookings, loading, refreshing, todayArrivals, todayStayovers, todayCheckouts, totalBookingsCount: apiTotalBookings, totalPages: apiTotalPages, uniqueProvinces: dataUniqueProvinces, uniqueCities: dataUniqueCities, uniqueCountries: dataUniqueCountries, refreshData } = useBusinessData(activeTab, currentPage, pageSize, currentFilters);
  const uniqueProvinces = dataUniqueProvinces?.length ? dataUniqueProvinces : _stateUniqueProvinces;
  const uniqueCities = dataUniqueCities?.length ? dataUniqueCities : _stateUniqueCities;
  const uniqueCountries = dataUniqueCountries?.length ? dataUniqueCountries : _stateUniqueCountries;
  ...
  const subscriptionTier = useMemo((): SubscriptionTier => {
    ...
    const planFields = [business.current_plan, business.plan, business.subscription_plan];
    for (const field of planFields) if (field) { const normalized = field.toLowerCase(); if (['starter','growth','pro','business','enterprise'].includes(normalized)) return normalized as SubscriptionTier; }
    const tier = business.subscription_tier?.toLowerCase() || '';
    if (['monthly','annual','trial','complimentary'].includes(tier)) { const rooms = business.total_rooms || 0; if (rooms >= 16) return 'business'; if (rooms >= 11) return 'pro'; if (rooms >= 6) return 'growth'; return 'starter'; }
    if (['starter','growth','pro','business'].includes(tier)) return tier as SubscriptionTier;
    const rooms = business.total_rooms || 0; if (rooms >= 16) return 'business'; if (rooms >= 11) return 'pro'; if (rooms >= 6) return 'growth'; return 'starter';
  }, [business]);
  ...
  const getStatusBadge = useCallback((status: string) => ({ checked_in:'bg-green-100 text-green-800', completed:'bg-blue-100 text-blue-800', confirmed:'bg-yellow-100 text-yellow-800', cancelled:'bg-red-100 text-red-800', pending:'bg-gray-100 text-gray-800' } as Record<string,string>)[status] || 'bg-gray-100 text-gray-800', []);
  ...
  const saveBusinessProfile = useCallback(async () => {
    ...
    try {
      const updateData = { businessId: business.id, total_rooms: parseInt(profileForm.total_rooms) || 0, avg_price: parseFloat(profileForm.avg_price) || 0, slogan: profileForm.slogan || '', welcome_message: profileForm.welcome_message || '', logo_url: profileForm.logo_url || business.logo_url || '', hero_image_url: profileForm.hero_image_url || business.hero_image_url || '' };
      ...
      alert(t('common_success')); setEditingProfile(false); refreshData();
    } catch (error) { console.error('Error saving profile:', error); alert(t('error_unexpected')); } finally { setSavingProfile(false); }
  ...
  const saveNewsletterSettings = useCallback(async () => {
    ...
    try {
      const newsletterData = { businessId: business.id, newsletter_enabled: newsletterEnabled, newsletter_title: newsletterTitle, newsletter_prize: newsletterPrize, newsletter_cta: newsletterCta, newsletter_terms: newsletterTerms, newsletter_draw_date: newsletterDrawDate || null, newsletter_share_text: newsletterShareText };
      const response = await fetch('/.netlify/functions/update-business-profile', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(newsletterData) });
      if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error || 'Failed to save newsletter settings'); }
      alert(t('common_success')); refreshData();
    } catch (error) { console.error('Error saving newsletter settings:', error); alert(t('error_unexpected')); } finally { setSavingNewsletter(false); }
  }, [business?.id, newsletterEnabled, newsletterTitle, newsletterPrize, newsletterCta, newsletterTerms, newsletterDrawDate, newsletterShareText, refreshData, setSavingNewsletter]);
  ...
  const handleTabChange = useCallback((tabId: string) => {
    setActiveTab(tabId); setCurrentPage(1);
    setSearchParams((prev) => { const next = new URLSearchParams(prev); next.set('tab', tabId); return next; });
  ...
  return <div className="min-h-screen bg-gray-50">
    <Header business={business} refreshing={refreshing} onRefresh={refreshData} onLogout={handleLogout} onShowQRModal={() => setShowQRModal(true)} />
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {activeTab === 'rooms' && <RoomsDashboardTab businessOverride={business} />}
    ...
    <DashboardModals showQRModal={showQRModal} showImportModal={showImportModal} showAppealModal={showAppealModal} business={business} rejectedRequest={rejectedRequest} onCloseQR={()=>setShowQRModal(false)} onCloseImport={()=>setShowImportModal(false)} onCloseAppeal={()=>{setShowAppealModal(false);setRejectedRequest(null);}} onImportComplete={()=>{refreshData();setShowImportModal(false);}} onAppealSubmit={refreshData} loadBookings={refreshData} fetchChangeRequests={refreshData} />
  </div>;
}
