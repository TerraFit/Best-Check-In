...
export default function HousekeepingSettings() {
  ...
  const addRoomType = () => {
    ...
    setTargets((current) => [...current, ...SERVICE_TYPES.map((serviceType) => ({ business_id: businessId, service_type: serviceType, room_type: roomType, target_minutes: findTarget(current, serviceType, null)?.target_minutes ?? 45, active: true }))]);
    setNewRoomType('');
  };
    ...
    <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      {loading || !settings || !serviceSettings ? <p className="text-center text-gray-400 text-sm py-12">{t('common_loading')}</p> : <>
        ...
        <section className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{SERVICE_TYPES.map((serviceType) => { const target = findTarget(targets, serviceType, null); return <label key={serviceType}><span className="block text-xs text-gray-500 mb-1">{SERVICE_LABELS[serviceType]}</span><div className="flex items-center gap-2"><input type="number" min={1} max={1440} value={target?.target_minutes ?? 1} onChange={(e) => updateTarget(serviceType, e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg" /><span className="text-xs text-gray-400">min</span></div></label>; })}</div>
        ...
        <section className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
          <div><h2 className="text-sm font-semibold text-gray-900">3. Temps par type de chambre</h2><p className="text-xs text-gray-500 mt-1">Les overrides sont utilisés uniquement pour les chambres dont le type correspond exactement.</p></div>
