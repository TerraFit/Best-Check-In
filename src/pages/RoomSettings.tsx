          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                ...
                <span className="text-orange-600">{licensedRooms !== null ? licensedRooms : '—'}</span>
              </p>
            </div>
            <div className="text-sm text-gray-500">
              {rooms.length > 0 && <span>{rooms.length} room{rooms.length === 1 ? '' : 's'} configured</span>}
            </div>
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {rooms.map((room) => {
                    ...
                    return (
                      <tr key={room.id} className={`${getRoomToneBorderClass(tone)} ${getRoomToneSurfaceClass(tone)}`}>
                        ...
                        <td className="px-4 py-3"><RoomStatusBadge room={room} /></td>
                        <td className="px-4 py-3">
                          {available ? (
                            <span className="inline-flex items-center gap-1 text-green-700 text-xs font-medium"><span aria-hidden>✅</span> {t('rooms_available')}</span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-red-700 text-xs font-medium"><span aria-hidden>❌</span> {room.unavailable_reason || t('rooms_unavailable')}</span>
                          )}
                        </td>
                        <td className="px-4 py-3">
                          <button type="button" onClick={() => openEdit(room)} className="text-xs font-medium text-orange-600 hover:text-orange-700">{t('common_edit')}</button>
                        </td>
                      </tr>
                    );
                  })}
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <p className="text-sm font-semibold text-gray-900 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">{editingRoom.room_number}</p>
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('rooms_room_name')}</label>
                <input type="text" value={form.room_name} onChange={(e) => setForm((f) => ({ ...f, room_name: e.target.value }))} placeholder={t('rooms_placeholder_name')} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500" />
                <p className="text-[11px] text-gray-400 mt-1">{t('rooms_shown_as')}{' '}<span className="font-medium text-gray-600">{getRoomDisplayName({ room_number: Number(editingRoom.room_number), room_name: form.room_name })}</span></p>
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('rooms_room_type')}</label>
                <select value={form.room_type} onChange={(e) => setForm((f) => ({ ...f, room_type: e.target.value }))} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500">
                  {!ROOM_TYPES.includes(form.room_type as any) && form.room_type && <option value={form.room_type}>{form.room_type}</option>}
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {([['max_adults', t('rooms_adults')], ['max_children', t('rooms_children')], ['max_infants', t('rooms_infants')]] as const).map(([key, label]) => (
                    <div key={key}>
                      <label className="block text-[10px] text-gray-400 mb-1">{label}</label>
                      <input type="number" min={0} max={20} value={form[key]} onChange={(e) => setForm((f) => ({ ...f, [key]: Math.max(0, parseInt(e.target.value, 10) || 0) }))} className="w-full px-2 py-2 text-sm border border-gray-300 rounded-lg" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="border border-gray-200 rounded-xl p-4 space-y-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.availableForAllocation} onChange={(e) => setForm((f) => ({ ...f, availableForAllocation: e.target.checked, unavailable_reason: e.target.checked ? '' : f.unavailable_reason }))} className="w-4 h-4 rounded border-gray-300 text-orange-500 focus:ring-orange-500" />
                ...
                {!form.availableForAllocation && (
                  <div>
                    ...
                    <select value={form.unavailable_reason} onChange={(e) => setForm((f) => ({ ...f, unavailable_reason: e.target.value }))} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg">
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} rows={3} placeholder={t('rooms_placeholder_notes')} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg resize-none" />
              </div>
